...
export class TerminalLinkManager extends DisposableStore {
	...
	private _getLinkHoverString(uri: string, label: string | undefined): IMarkdownString {
		// Maut code: when an external link provider tags its tooltip with the `mautImg:` prefix,
		// render the rest as raw markdown so we can embed an image preview in the hover.
