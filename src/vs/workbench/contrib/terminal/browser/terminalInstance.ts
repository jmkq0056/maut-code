...
export class TerminalInstance extends Disposable implements ITerminalInstance {
	...
	private _initDragAndDrop(container: HTMLElement) {
		...
		store.add(dndController.onDropText(async droppedText => {
			...
			const activeCodeEditor = this._editorService.activeTextEditorControl as {
				getSelection?: () => { startLineNumber: number; endLineNumber: number } | null;
				getModel?: () => { uri: URI } | null;
			} | undefined;
			const sel = activeCodeEditor?.getSelection?.();
			const model = activeCodeEditor?.getModel?.();
			if (sel && model && model.uri.scheme === 'file') {
				const folder = this._workspaceContextService.getWorkspaceFolder(model.uri);
				const rel = folder ? model.uri.fsPath.substring(folder.uri.fsPath.length).replace(/^\/+/, '') : model.uri.fsPath;
				const start = sel.startLineNumber;
				const end = sel.endLineNumber;
				const ref = start === end ? `@${rel}:${start} ` : `@${rel}:${start}-${end} `;
				this.focus();
				await this.sendText(ref, false);
				return;
			}
			// Fallback: paste the raw text.
			await this.sendText(droppedText, false);
		}));
...
class TerminalInstanceDragAndDropController extends Disposable implements dom.IDragAndDropObserverCallbacks {
	...
	private readonly _onDropText = this._register(new Emitter<string>());
	get onDropText(): Event<string> { return this._onDropText.event; }
